module Capistrano
  module DSL
    ...
    def _uploader_factory
      ->(*args) { Template::Helpers::Uploader.new(*args) }
    end

    def _paths_factory
      ->(*args) { Template::Helpers::PathsLookup.new(*args) }
    end
    ...
    def _template_factory
      ->(from, context, digester, locals) { Template::Helpers::TemplateDigester.new(Template::Helpers::Renderer.new(from, context, locals: locals), digester) }
    end
